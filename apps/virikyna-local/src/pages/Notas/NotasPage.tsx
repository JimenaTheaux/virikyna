...
export function NotasPage() {
  ...
  async function cargar() {
    setLoading(true)
    setError(null)
    // Vista con el nombre del autor ya resuelto (docs/16_notas_internas.sql) — un cajero no tiene
    // acceso directo a la fila de `perfiles` de otro usuario, así que el join no se puede hacer acá.
    // Filtro explícito por origen='local' (docs/06_estructura_de_datos (1).md sección 15): aunque
    // RLS ya bloquea 'gestion' para un cajero, Local nunca debe pedir ni mostrar esas notas, ni
    // siquiera para un admin que esté usando esta app.
